...
void main() {
  group('AsyncButtonState equality', () {
    test('idle equals idle', () {
      ...
      expect(const AsyncButtonState.idle().hashCode,
          const AsyncButtonState.idle().hashCode);
    });

    test('loading equals loading', () {
      expect(
          const AsyncButtonState.loading(), const AsyncButtonState.loading());
    });

    test('success equals success', () {
      expect(
          const AsyncButtonState.success(), const AsyncButtonState.success());
    });
    ...
    test('error equals error by error object', () {
      ...
      expect(AsyncButtonState.error(Exception('a')) ==
          AsyncButtonState.error(Exception('a')),
          isFalse,
    ...
    test('different variants are not equal', () {
      expect(
          const AsyncButtonState.idle() == const AsyncButtonState.loading(),
          isFalse);
      expect(
          const AsyncButtonState.success() == const AsyncButtonState.idle(),
          isFalse);
    });
  });
  ...
  group('AsyncButtonState toString', () {
    test('readable identifiers', () {
      expect(const AsyncButtonState.idle().toString(),
          'AsyncButtonState.idle()');
      expect(const AsyncButtonState.loading().toString(),
          'AsyncButtonState.loading()');
      expect(const AsyncButtonState.success().toString(),
          'AsyncButtonState.success()');
      expect(AsyncButtonState.error('boom').toString(),
          'AsyncButtonState.error(boom)');
    });
  });
